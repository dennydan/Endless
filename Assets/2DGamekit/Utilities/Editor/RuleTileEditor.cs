...
{
	[CustomEditor(typeof(RuleTile2DGK))]
	...
	internal class RuleTileEditor : Editor
	{
		...
		public RuleTile2DGK tile { get { return (target as RuleTile2DGK); } }
		...
		public void OnEnable()
		{
			if (tile.m_TilingRules == null)
				tile.m_TilingRules = new List<RuleTile2DGK.TilingRule>();

			m_ReorderableList = new ReorderableList(tile.m_TilingRules, typeof(RuleTile2DGK.TilingRule), true, true, true, true);
		...
		{
            RuleTile2DGK.TilingRule rule = tile.m_TilingRules[index];
		...
		private static void RuleMatrixOnGUI(Rect rect, RuleTile2DGK.TilingRule tilingRule)
		{
			...
			for (int y = 0; y <= 2; y++)
			{
				for (int x = 0; x <= 2; x++)
				{
					...
					if (x != 1 || y != 1)
					{
						...
						{
							case RuleTile2DGK.TilingRule.Neighbor.This:
								GUI.DrawTexture(r, arrows[y*3 + x]);
								break;
							case RuleTile2DGK.TilingRule.Neighbor.NotThis:
								GUI.DrawTexture(r, arrows[9]);
								break;
						}
						if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
						{
							tilingRule.m_Neighbors[index] = (RuleTile2DGK.TilingRule.Neighbor) (((int)tilingRule.m_Neighbors[index] + 1) % 3);
							GUI.changed = true;
							Event.current.Use();
						}
					...
					else
					{
						...
						{
							case RuleTile2DGK.TilingRule.AutoTransform.Rotated:
								GUI.DrawTexture(r, autoTransforms[0]);
								break;
							case RuleTile2DGK.TilingRule.AutoTransform.MirrorX:
								GUI.DrawTexture(r, autoTransforms[1]);
								break;
							case RuleTile2DGK.TilingRule.AutoTransform.MirrorY:
						...
						if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
						{
							tilingRule.m_AutoTransform = (RuleTile2DGK.TilingRule.AutoTransform)(((int)tilingRule.m_AutoTransform + 1) % 4);
							GUI.changed = true;
							Event.current.Use();
						}
					}
		...
		{
			public RuleTile2DGK.TilingRule m_Rule;
			public RuleTile2DGK.TilingRule.AutoTransform m_NewValue;

			public MenuItemData(RuleTile2DGK.TilingRule mRule, RuleTile2DGK.TilingRule.AutoTransform mNewValue)
			{
				this.m_Rule = mRule;
		...
		private void SpriteOnGUI(Rect rect, RuleTile2DGK.TilingRule tilingRule)
		{
		...
		private static void RuleInspectorOnGUI(Rect rect, RuleTile2DGK.TilingRule tilingRule)
		{
			float y = rect.yMin;
			EditorGUI.BeginChangeCheck();
			GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Rule");
			tilingRule.m_AutoTransform = (RuleTile2DGK.TilingRule.AutoTransform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_AutoTransform);
			...
			tilingRule.m_Output = (RuleTile2DGK.TilingRule.OutputSprite)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Output);
			y += k_SingleLineHeight;

			if (tilingRule.m_Output == RuleTile2DGK.TilingRule.OutputSprite.Animation)
			{
			...
			if (tilingRule.m_Output == RuleTile2DGK.TilingRule.OutputSprite.Random)
			{
			...
			if (tilingRule.m_Output != RuleTile2DGK.TilingRule.OutputSprite.Single)
			{
